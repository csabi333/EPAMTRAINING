class Main{
    constructor(profileService,postService,commentService){
        ...
        this.template=`<main><h1>My posts</h1><div id="main">Loading...</div></main>`;
    }
    ...
    fetchPosts(top,skip){
        ...
        return this.profileService.fetchProfile()
            ...
            .then(function(id){
            return that.postService.fetchPosts(id, top, skip)
        })
            .then(function(posts){
            that._renderPosts(posts);
            return posts;
        })
            .then(function(posts){
                var comments = posts.map(function(post){
                    return that.commentService.fetchComments(post.id)
                    .then(function(comments){
                        that._renderComments(comments);
                        });
                                        })
...
const main = new Main(ProfileService,PostService,CommentService);
